...
/**
 * Service for interacting with the insights/trend endpoint
 */
export class TrendService {
    /**
     * Fetch trend data for a specific metric and resident
     * @param {string} metric - The metric type (must be one of METRIC_TYPES)
     * @param {number} residentId - The resident ID
     * @returns {Promise<TrendData>} The trend data response
     * @throws {Error} If metric is invalid or API call fails
     */
    static async getTrendData(metric, residentId) {
        ...
        const endpoint = `/api/insights/trend/${metric}/${residentId}`;
...
export const getTrendData = (metric, residentId) => {
    return TrendService.getTrendData(metric, residentId);
};