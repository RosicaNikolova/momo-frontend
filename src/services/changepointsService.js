...
/**
 * Service for interacting with the insights/changepoints endpoint
 */
export class ChangepointsService {
    /**
     * Fetch changepoints data for a specific metric and resident
     * @param {string} metric - The metric type (must be one of METRIC_TYPES)
     * @param {number} residentId - The resident ID
     * @returns {Promise<ChangepointsData>} The changepoints data response
     * @throws {Error} If metric is invalid or API call fails
     */
    static async getChangepointsData(metric, residentId) {
        ...
        const endpoint = `/api/insights/changepoints/${metric}/${residentId}`;
...
/**
 * Convenience function to get changepoints data
 * @param {string} metric - The metric type
 * @param {number} residentId - The resident ID
 * @returns {Promise<ChangepointsData>} The changepoints data response
 */
export const getChangepointsData = (metric, residentId) => {
    return ChangepointsService.getChangepointsData(metric, residentId);
};